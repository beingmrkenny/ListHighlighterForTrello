...
function releaseZip(forFirefox) {
	...
	try { fs.emptyDirSync(`/tmp/${extensionName}`); } catch (err) { console.log(err); }
	try { fs.copySync('Extension', `/tmp/${extensionName}`); } catch (err) { console.log(err); }
	...
	child_process.execSync(`zip -r ${process.env.HOME}/Desktop/${zipFileName} *`, { cwd: `/tmp/${extensionName}` });
