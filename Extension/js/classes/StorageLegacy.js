...
class StorageLegacy {
	...
	static processColors (results) {
		...
		for (let propName in results) {
			if (propName.startsWith('colors.custom.')) {
				colors.custom[propName.replace('colors.custom.', '')] = results[propName];
			}
			if (propName.startsWith('colors.current.')) {
				colors.current[propName.replace('colors.current.', '')] = results[propName];
			}
		}
		return colors;
	}
	...
	static mapV2OptionsToV4Options (v2Options) {
		...
		var v4Options = {},
			v2toV4 = {
				...
				'options.HeaderCardsExtraSpace':     'options-OrganisingHeaderCardsExtraSpace',
				'options.EnableSeparatorCards':      'options-OrganisingEnableSeparatorCards',
				'options.SeparatorCardsVisibleLine': 'options-OrganisingSeparatorCardsVisibleLine'
			};
	...
	static makeRulesFromV2Options (results) {
		...
		var highlightTags        = (options.hasOwnProperty('HighlightTags') && options.HighlightTags               == true),
			highlightTitles      = (options.hasOwnProperty('HighlightTitles') && options.HighlightTitles           == true),
			matchTitleSubstrings = (options.hasOwnProperty('MatchTitleSubstrings') && options.MatchTitleSubstrings == true),
			dimUntaggedNormal    = (options.hasOwnProperty('DimUntaggedNormal') && options.DimUntaggedNormal       == true),
			dimUntaggedHigh      = (options.hasOwnProperty('DimUntaggedHigh') && options.DimUntaggedHigh           == true),
			dimmingLow           = (options.hasOwnProperty('DimmingLow') ? options.DimmingLow : 1),
			dimmingDone          = (options.hasOwnProperty('DimmingDone') ? options.DimmingDone : 1);
