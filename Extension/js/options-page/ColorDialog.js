class ColorDialog {
	static setup(color) {
		...
		listen(
			qq('[name="ColorTile"]'),
			'change',
			ColorDialog.colorTileChangeListener
		);
	...
	static colorTileChangeListener() {
		...
		ColorDialog.updateDemoListColor(
			ColorDialog.getListColor(this.value),
			qid('ColorTile-normal').disabled
		);
	...
	static editCustomColor() {
		...
		new ColorPickerWrapper({
			...
			save: () => {
				q('.color-tile-bar').dataset.lastValidColor =
					qid('ColorTile-custom').value;
			},
			cancel: () => {
				let lastValidColor =
					this.closest('.color-tile-bar').dataset.lastValidColor;
				qid('ColorTile-custom').value = this.dataset.colorOnOpen;
				setBackgroundColor(
					q('[for="ColorTile-custom"]'),
					this.dataset.colorOnOpen == ''
						? null
						: ColorDialog.getListColor(this.dataset.colorOnOpen)
				);
				ColorDialog.updateDemoListColor(
					lastValidColor,
					qid('ColorTile-normal').disabled
