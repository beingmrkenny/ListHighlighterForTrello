...
class ListWorkPoints {
	...
	static updateLists (lists) {
		...
		for (let cardElement of qq(ListWorkPoints.getCardSelector())) {
			let card = new HeaderSeparatorCard(cardElement);
			if (Global.getItem('options-CountEnableWIP')
				&& Global.getItem('options-CountEnablePointsOnCards')
				&& Global.getItem('options-CountHideManualCardPoints')
				&& !card.isHeader()
				&& !card.isRule()
			) {
	...
	update () {
		...
		if (Global.getItem('options-CountEnableWIP')) {
			...
			if (!Global.getItem('options-CountAllCards') && typeof listLimit == 'undefined') {
				remove = true;
			}
	...
	getCardCount() {
		...
		if (Global.getItem('options-CountEnablePointsOnCards')) {
			points = 0;
	...
	updateCountAndLimit (cardCount, listLimit) {
		...
		if (typeof listLimit != 'undefined' || Global.getItem('options-CountAllCards')) {
			...
			var name = (Global.getItem('options-CountEnablePointsOnCards') && cardCount.points != cardCount.cardCount) ? 'point' : 'card',
				s = (countOnList == 1) ? '' : 's',
				cardCountString = `${countOnList} ${name}${s}`;

			if (listLimit === '#count' || (Global.getItem('options-CountAllCards') && typeof listLimit == 'undefined')) {
